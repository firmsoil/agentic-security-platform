...
Today, the platform maps findings to the OWASP Top 10 for LLM Applications (2025) and the OWASP Agentic Top 10 (2026). It is Apache 2.0 licensed, intentionally built in the open, and designed to be legible both to contributors and to the teams who may eventually rely on it in regulated environments. Concrete launch evidence — the adversarial false-positive sweep results, the per-target reconciliation runs, and the architectural decisions that shaped this release — is committed under [`docs/evidence/`](evidence/), [`docs/adr/`](adr/), and [`docs/llm-scanner.md`](llm-scanner.md). The intent is that anyone evaluating the project can see not just what the code does but what we tested before saying so.
...
We validated the contract empirically before shipping. Across nine randomly-chosen non-AI public repositories spanning all three stacks — Python utilities (Black, Click, Bottle), Spring CRUD samples (spring-petclinic, spring-mvc-showcase, gs-rest-service), and Node libraries (chalk, swr, create-vue) — the LLM scanner produced zero false-positive Tool/PromptTemplate/RAGIndex/MemoryStore nodes. The verifier correctly distinguished application code from AI-tool-shaped code on every one. The sweep log lives under [`docs/evidence/`](evidence/) for inspection. Real targets were also reconciled to strict-match parity in one or two passes: `langchain4j-examples/customer-support-agent-example` (Java, eight nodes confirmed including two `@Tool`-annotated methods) and `vercel-labs/ai-sdk-preview-rag` (Node, eight nodes confirmed including three Vercel AI SDK tool factories and a Drizzle pgvector embedding store).

The bright line is deliberate: the LLM produces *inventory*, not findings — the security-semantic edges (`PROMPT_INJECTABLE_INTO`, `TOOL_INVOKABLE_BY`, `MEMORY_POISONABLE_BY`, …) live in hand-authored per-target profiles and are not LLM-generated.
...
With the multi-stack scanner and the LLM-assisted source extractor shipping in this release, the next chapter is about turning the current demo into a more continuously observed system. The launch lands the static side of the story — what attack paths can be discovered from a repository at rest. What follows extends the same graph into runtime.
