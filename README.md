...
> **Status: v0.2 — multi-stack scanner shipped, LLM-assisted source extractor cleared for launch.** The GitHub connector scans Python, Java/Spring, and Node/TypeScript repositories. The dependency-manifest pass is deterministic per stack; the four code-shape node types (`Tool`, `PromptTemplate`, `RAGIndex`, `MemoryStore`) come from a second pass that uses an LLM under the [grounding contract recorded in ADR-0005](docs/adr/0005-llm-scanner-grounding-contract.md). Two real-world targets — [`langchain4j-examples/customer-support-agent-example`](targets/customer-support-agent.yaml) and [`vercel-labs/ai-sdk-preview-rag`](targets/ai-sdk-preview-rag.yaml) — reconciled to strict-match parity, and an adversarial sweep across nine non-AI repositories produced zero false-positive extractions ([sweep log](docs/evidence/adversarial-sweep-2026-04-29.log)). Tenant scoping is enforced, the Cypher injection chokepoint is in place, and the seeded demo graph powers live attack-path queries plus a frontend graph explorer. The canonical prompt-injection path traverses an explicit `Prompt` node (`Source -> PROMPT_INJECTABLE_INTO -> Prompt <- USES_PROMPT - Model`) before reaching tool and data sinks. Phase 1 ingestion remainder (Redpanda event bus + async graph workers, OTel→graph ingestion, AWS connector) stays planned per the [launch roadmap](docs/launch-roadmap.md).
